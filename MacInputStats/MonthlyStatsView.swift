...
struct MonthlyStatsView: View {
    ...
    private var inputGrid: some View {
        LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 12) {
            StatCard(title: "Keystrokes", value: formatted(totalKeystrokes), icon: "keyboard")
            StatCard(title: "Clicks", value: formatted(totalClicks), icon: "cursorarrow.click.2")
            StatCard(title: "Scrolls", value: formatted(totalScrolls), icon: "scroll")
            StatCard(title: "Talk Time", value: AppStats.formatDuration(totalTalkSeconds), icon: "waveform")
        }
    }
